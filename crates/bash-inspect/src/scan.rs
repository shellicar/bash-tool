//! Finding the refused constructs in the source, with their positions.
//!
...
struct ScanWord {
    /// Quoting still in place, exactly as the lexer saw it: `"set"` is four
...
pub fn scan(source: &str) -> Vec<(Construct, usize)> {
    ...
    while let Ok((token, at)) = lexer.next_token_at() {
        match token {
            Token::Word(text, quoted) => {
                ...
                if redirect_target {
                    ...
                    current.push(ScanWord { text, quoted, at });
                }
            }
            Token::DLess => {
...
fn check(words: &[ScanWord], found: &mut Vec<(Construct, usize)>) {
    let mut i = 0;
    while let Some(word) = words.get(i) {
        if (!word.quoted && OPENERS.contains(&word.text.as_str()))
    ...
    if !word.quoted && word.text == "select" {
        found.push((Construct::Select, word.at));
    } else if !word.quoted && word.text == "coproc" {
        found.push((Construct::Coproc, word.at));
    } else if literal(&word.text).as_deref() == Some("set")
        && let Some(at) = posix_option(&words[i + 1..])
    {
        found.push((Construct::PosixMode, at));
    }
}
...
fn posix_option(args: &[ScanWord]) -> Option<usize> {
    let mut i = 0;
    while let Some(arg) = args.get(i) {
        let flag = literal(&arg.text)?;
